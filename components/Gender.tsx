...
const Gender: FC<ChildComponentProps> = ({ onGender, showError, gender }) => {
  ...
  return (
    <section>
      ...
      <label className="text-3xl leading-loose font-semibold grid grid-cols-radio gap-2 sm:text-2xl mb-5 xl:text-3xl xl:mb-10">
        <input type="radio" id="male" name="gender" value="male" onChange={handleSelect} checked={gender == "male" ? true : false} />
        Male
      </label>
      <label className="text-3xl leading-loose font-semibold grid grid-cols-radio gap-2 sm:text-2xl xl:text-3xl">
